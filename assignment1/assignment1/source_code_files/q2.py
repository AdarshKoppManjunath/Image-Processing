...
python q3.py "input_imag_path" "output_image_path" inputimage_col inputimage_row outputimage_col outputimage_row

q2.py : is the file name
...
def read_image_into_array(file_name,input_cols,input_rows,output_cols,output_rows):
    ...
    input_image_array.shape = (input_image_array.size//input_cols,input_cols) #1D to 2D
...
def resize(old_image, new_image, input_cols, input_rows, output_cols, output_rows,destination_path):
    ...
    This function caluclates the intervals and interval length and finally call the resample function for the x and y position
    Resizing is done here using weighted average resampling method.
    ...
    for row in range(output_rows):
        ...
        for col in range(output_cols):
           ...
           sample_inter= int(sample/2)
           sum=0.0
           for index_s in mit.numeric_range(-sample_inter,sample_inter):
        
                for index_t in mit.numeric_range(-sample_inter,sample_inter):

                    if resample_x+sample_inter*interval_length >=(input_cols-1) or resample_y+sample_inter*interval_length>=(input_rows-1): 
                      #avoid accessing input image array when resample postions greater than input image rows and cols
                      break


                    sum+=resampling(old_image,resample_x+sample_inter*interval_length,resample_y+sample_inter*interval_length)

           new_image[row][col]=sum/(sample*sample)
           resample_x += Dx
    ...
    new_image.astype("int8").tofile(destination_path) #saving the image into output file
...
if __name__ == "__main__":   
    ...
    python q2.py inputimage_path outputimage_path inputimage_col inputimage_row outputimage_col outputimage_row"""
