...
python part1.py "<input_imag_path>"  <inputimage_col> <inputimage_row> "<outputimage_path>"

part1.py : is the file name
...
def caluclate_magnitude(dft):
    """This function is used to compute magnitude of dft
    Param:
        dft(ndarray): fft applied on both row and col, combined
    Return:
        mag(ndarray): magnitude values float ndarray
        max(float): used for scaling"""

    #Compute the magnitude of dft[][]
    rows,cols=dft.shape
    mag=np.zeros((rows,cols),dtype=float)
    max=0.0
    for i in range(rows):
            for j in range(cols):
                mag[i][j] = float(abs(dft[i][j]))   #caluclates the magnitude of a complex number same as math.sqrt(dft[i][j].real * dft[i][j].real + dft[i][j].imag * dft[i][j].imag
                if (mag[i][j] > max):
                    max = mag[i][j]
...
def scale(mag):
    """This function is used to scale and copy the magnitude to output
    Params:
        mag(nadarray)- magnitude
    Return:
        out(ndarray)- scaled magnitude"""

    #scale the value into the range [0, 255], and store the result in out_img[][]
    rows,cols=mag.shape
    out=np.zeros((rows,cols),dtype=int)
    scale_factor = 255.0 // math.log(1.0 + max)
    print(scale_factor)
    print(mag)
    for i in range(rows):
            for j in range(cols):
                mag[i][j] = (float)(scale_factor*(1.0*math.log(1.0 + mag[i][j])))
                out[i][j] = int(mag[i][j])
...
def display_and_save_images(ou,destination_path):
    """This function will display the image and save ndarray into image
    Params:
        ou(ndarray): scaled ndarray after computing magnitude from dft
        destination path(str): path where image needs to be saved"""

    plt.subplot(111)
    plt.imshow(ou,'gray') # display the image. 
    plt.title('result')
    plt.show()
    #print(image)
    destination_path=destination_path+"\\part1_car.raw"
    ou.astype("int8").tofile(destination_path) #save ndarray into image
    return True


def dft_to_binaryfile_for_future_use(dft,destination_path):
    """This function saves dft[][] into a file in binary format for future use as mentioned in the assiginment 4 part 1

    Params:
       dft(ndarray): dft ndarry of type complex"""

    destination_path=destination_path+"\\part1_cardft.bin"
    dft.tofile(destination_path) #saves the dft into a file in binary format ( tofile function of numoy saves file in binary format by default)
...
if __name__ == "__main__":    
    ...
    if len(sys.argv)<5:
        print("command format should be ---- \n python part1.py inputimage_path  inputimage_col inputimage_row outputimage_path\n")
    else:
        ...
        mag,max=caluclate_magnitude(dft)
        out=scale(mag)
        display_and_save_images(out,sys.argv[4])
        dft_to_binaryfile_for_future_use(dft,sys.argv[4])
