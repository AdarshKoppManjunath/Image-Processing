...
python  lowerpassfilter.py "<input_imag_path>"  <inputimage_col> <inputimage_row> "<outputimage_path>"

lowerpassfilter : is the file name
...
def ideal_low_pass_filter(D0,img_shape):
    f = np.zeros(img_shape[:2])  #initialize base with zeros (filter)
    rows, cols = img_shape[:2]      #image size
    center = (rows/2,cols/2) #center poistion
    #distance to the center is caluclated for all the points. If its less than cut off then set it  to 1
    for i in range(cols): 
        for j in range(rows):
...
def ifft2d(lowpassfiltered_image):
    """This function is used to apply inverse fft 
    Param:
        lowpassfiltered_image(ndarray): fft*lf
    Return: 
        idft(ndarray): inverse fft"""
    
    rows,cols=lowpassfiltered_image.shape

    idft=np.zeros((rows,cols),dtype=complex)

   # we are conjugating the fft*lf  and applying fft2d on that 
    for i in range(rows):
        for j in range(cols):
            lowpassfiltered_image[i][j]=np.conjugate(lowpassfiltered_image[i][j])
    
    idft=fft2d(lowpassfiltered_image)
...
def display_and_save_images(mag_idft,destination_path):
    ...
    Params:
        mag_idft(ndarray): magnitude of idft
    ...
    plt.show()
    #print(image)
    destination_path=destination_path+"\\lf_car_cutoff_30.raw"
...
if __name__ == "__main__":    
    ...
    if len(sys.argv)<5:
        ...
        print("command format should be ---- \n python lowerpassfilter.py inputimage_path  inputimage_col inputimage_row outputimage_path\n")
    else:
        ...
        #low pass filter
        D0=10 #cut off
        lf=ideal_low_pass_filter(D0,[256,256])
        
       
        #display low pass filter
        plt.subplot(111)
        plt.imshow(abs(lf), "gray")
        plt.title("Ideal Low Pass")
        plt.show()

        #apply low pass filter on caluclated fft
        print("\n\nlow pass filter with the cut off frequency - 30 \n \n", lf)
        LowPassCenter = dft * lf
        print("\n\n fft * lf \n \n", LowPassCenter)

        #apply inverse fft on lowpasscenter
        idft=ifft2d(LowPassCenter)
        print("\nidft \n",idft)
